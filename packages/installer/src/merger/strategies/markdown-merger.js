/**
 * @fileoverview Merge strategy for Markdown files with AIOS-MANAGED sections
 ...
 * Uses AIOS-MANAGED markers to identify sections that can be updated.
...
class MarkdownMerger extends BaseMerger {
  ...
  /**
   * Can merge if existing content has AIOS markers
   ...
   * - Updates AIOS-MANAGED sections with new content
   * - Preserves user sections (without markers)
   * - Adds new AIOS sections at the end
   * @param {string} existingContent - Existing markdown content
  ...
  async merge(existingContent, newContent, options = {}) {
    ...
    // If existing file has no AIOS markers, do legacy migration
    ...
    for (const section of existing.sections) {
      if (section.managed) {
        ...
        if (templateSection) {
          ...
          merged += `<!-- AIOS-MANAGED-START: ${section.id} -->\n`;
          merged += templateSection.lines.join('\n');
          if (!merged.endsWith('\n')) {
            merged += '\n';
          }
          merged += `<!-- AIOS-MANAGED-END: ${section.id} -->\n\n`;
    ...
    if (newSections.length > 0) {
      merged += '---\n\n';
      merged += '<!-- New AIOS sections added -->\n\n';
  ...
  /**
   * Migrate a legacy file (without AIOS markers) by appending AIOS sections
  ...
  async migrateLegacy(existingContent, template, _options = {}) {
    ...
    // Add separator and AIOS sections
   ...
   */
  getDescription() {
    return 'Merges Markdown files using AIOS-MANAGED section markers';
  }
}
